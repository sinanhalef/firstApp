...
const SpyCreateYourOwn = () => {
	...
	const addWord = () => {
		if (word.trim() && !words.includes(word.trim())) {
			alreadySaved && setAlreadySaved(false);
	...
	const saveSetAndPlay = async () => {
		...
		try {
			...
			const rawTitle = Array.isArray(saveName) ? saveName[0] : saveName || (Array.isArray(params.wordSet) ? params.wordSet[0] : params.wordSet) || 'My Set';

			const wordSetTitle = typeof rawTitle === 'string' ? rawTitle.replace('.json', '') : 'My Set';
			await update(ref(db, `rooms/${params.roomCode}`), {
				wordSet: '',
				wordSetTitle,
				selectedSetKey: '',
	...
	const saveSetWithName = async () => {
		...
		try {
			...
			const nameToSave = (saveName === "" || saveName.trim() === "" || saveName === null)
				? (params.wordSet? params.wordSet as string : 'My Set')
				: saveName.trim();
		...
		return (
				<View style={styles.container}>
					<TouchableOpacity style={styles.backIcon} onPress={() => router.push({ pathname: '/spy_multiplayer/spy_my_sets', params: { roomCode: params.roomCode } })}>
