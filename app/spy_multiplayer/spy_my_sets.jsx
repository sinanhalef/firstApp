...
const SpyMySets = () => {
  ...
  const deleteSet = async (filename) => {
    try {
      await FileSystem.deleteAsync(FileSystem.documentDirectory + "spy/"+ filename);
  ...
  useEffect(() => {
    const loadSets = async () => {
      const files = await FileSystem.readDirectoryAsync(FileSystem.documentDirectory + "spy/");
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.backIcon} onPress={() => router.push({ pathname: '/spy_multiplayer/spy_multiplayer_game_select', 
      ...
      <TouchableOpacity
                    ...
                    onPress={() => { router.push({ pathname: '/spy_multiplayer/spy_create_your_own', params: { wordSet: "", roomCode: params.roomCode } }); }}
                >
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.setRow}>
            ...
            onPress={() => { router.push({ pathname: '/spy_multiplayer/spy_create_your_own', params: { wordSet: item, roomCode: params.roomCode } }); }}>
